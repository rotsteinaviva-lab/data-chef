...
2. In the left-hand pane under **Connect**, click **To a File** $\rightarrow$ **More...**
...
4. Because the field names differ, Tableau will prompt you to define the relationship manually. Under the relationship settings, map the fields together by setting:
   $$\text{Communities (from the Master List)} = \text{Final-Names (from the Historical Dataset)}$$
...
1. Go back to your worksheet (**Sheet 1**) and double-click the sheet tab at the bottom to rename it: `Calgary Population Viz`.
...
3. Switch back to your `Calgary Population Viz` worksheet tab. Tableau will prompt you to save the local data extraction. Save the `.hyper` file in an accessible directory on your computer.
