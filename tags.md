...
<!-- 1. The Interactive Filter Cloud -->
...
<div class="tags-lists">
  {% for tag in sorted_tags %}
    ...
    <div id="{{ tag_name | downcase | slugify }}" style="margin-bottom: 45px; scroll-margin-top: 30px;">
      <h2 style="border-bottom: 2px solid #7a8288; padding-bottom: 8px; color: #1a2530;">
        📁 {{ tag_name }}
      </h2>
      <ul style="list-style-type: square; padding-left: 20px;">
        {% for post in tag_posts %}
          <li style="margin-bottom: 10px;">
            <a href="{{ post.url | relative_url }}" style="font-weight: bold; text-decoration: none; font-size: 1.1rem;">
              {{ post.title }}
            </a>
